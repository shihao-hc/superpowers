// Phase D: Extended adapters test (more symbols across US/CN/HK)
(async () => {
  try {
    ...
    const usInput = ['AAPL','MSFT','GOOGL','AMZN','TSLA']
    const cnInput = ['600519','000001','601318','600036','000651']
    const hkInput = hkSymbols && hkSymbols.length ? hkSymbols.slice(0, 4) : ['0700.HK','9988.HK','1833.HK','2318.HK']
    ...
    if (ok && usRes.length === usInput.length && cnRes.length === cnInput.length && hkRes.length === hkInput.length) {
      console.log('Phase D extended 3 test passed')
      process.exit(0)
    } else {
      console.error('Phase D extended 3 test failed: mismatched lengths', { usRes, cnRes, hkRes })
      process.exit(1)
    }
  } catch (e) {
    console.error('Phase D extended 3 test exception', e)
    process.exit(1)
  }
})()
