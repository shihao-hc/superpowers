// Phase D: Extended adapters test (US/CN/HK - set 6)
(async () => {
  try {
    ...
    const usInput = ['AAPL','MSFT','GOOGL','AMZN','NVDA','TSLA']
    const cnInput = ['600519','000001','601318','600036','000651','600276']
    ...
    if (ok && usRes.length === usInput.length && cnRes.length === cnInput.length && hkRes.length === hkInput.length) {
      console.log('Phase D extended 6 test passed')
      process.exit(0)
    } else {
      console.error('Phase D extended 6 test failed: mismatched lengths', { usRes, cnRes, hkRes })
      process.exit(1)
    }
  } catch (e) {
    console.error('Phase D extended 6 test exception', e)
    process.exit(1)
  }
})()
