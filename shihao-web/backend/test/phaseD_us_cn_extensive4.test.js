// Phase D: US/CN extended extensively (test set 4)
(async () => {
  try {
    ...
    const usInput = ['AAPL','MSFT','GOOGL','AMZN','NVDA','TSLA','META']
    ...
    if (ok && usRes.length === usInput.length && cnRes.length === cnInput.length && hkRes.length === hkInput.length) {
      console.log('Phase D US/CN extended 4 test passed')
      process.exit(0)
    } else {
      console.error('Phase D US/CN extended 4 test failed: mismatched lengths', { usRes, cnRes, hkRes })
      process.exit(1)
    }
  } catch (e) {
    console.error('Phase D US/CN extended 4 test exception', e)
    process.exit(1)
  }
})()
