// Phase D: US/CN extended extensively (test set 3)
(async () => {
  try {
    ...
    const usInput = ['AAPL','MSFT','GOOGL','AMZN','NVDA','INTC']
    const cnInput = ['600519','000001','601318','600036','000651','600000']
    const hkInput = hkSymbols && hkSymbols.length ? hkSymbols.slice(0, 6) : ['0700.HK','9988.HK','1833.HK','2318.HK','3888.HK','2338.HK']
    ...
    if (ok && usRes.length === usInput.length && cnRes.length === cnInput.length && hkRes.length === hkInput.length) {
      console.log('Phase D US/CN extended 3 test passed')
      process.exit(0)
    } else {
      console.error('Phase D US/CN extended 3 test failed: mismatched lengths', { usRes, cnRes, hkRes })
      process.exit(1)
    }
  } catch (e) {
    console.error('Phase D US/CN extended 3 test exception', e)
    process.exit(1)
  }
})()
