// Phase D: Extended adapters test (US/CN/HK - set 5)
(async () => {
  try {
    ...
    if (ok && usRes.length === usInput.length && cnRes.length === cnInput.length && hkRes.length === hkInput.length) {
      console.log('Phase D extended 5 test passed')
      process.exit(0)
    } else {
      console.error('Phase D extended 5 test failed: mismatched lengths', { usRes, cnRes, hkRes })
      process.exit(1)
    }
  } catch (e) {
    console.error('Phase D extended 5 test exception', e)
    process.exit(1)
  }
})()
