...
function bb(a){
  var clean = a.replace(/^"(.*)"$/, '$1');
  var argsList = ['brain-bridge.js'].concat(clean.split(/\s+/));
  var r = cp.spawnSync('node', argsList, { encoding: 'utf8', timeout: 15000 });
...
assert('C','DecisionContext.generate()',function(){
  var DC=require('./src/core/DecisionContext');
  var d=new DC();
  var r=d.generate('test','code',[{id:'l1',title:'x',priority:'high'}],{interactionCount:5});
...
assert('C','DecisionContext risk=high for security',function(){
  var DC=require('./src/core/DecisionContext');
  var d=new DC();
  var r=d.generate('test','security',[],{});
...
assert('C','DecisionContext recommendations by type',function(){
  ...
  // 应该包含安全相关的建议
  var hasSec=r.recommendations.some(function(s){return s.indexOf('漏洞')>=0||s.indexOf('验证')>=0||s.indexOf('安全')>=0});
...
assert('C','DecisionTracker record & history',function(){
  var DT=require('./src/core/DecisionTracker');
  var d=new DT();
  var rec=d.record({input:'test',taskType:'code',decision:'processed',riskLevel:'low'});
  if(!rec.id)throw'no id';
  var h=d.getHistory(5);
...
assert('C','DecisionTracker getRecentLessons',function(){
  ...
  var r=d.getRecentLessons(5);
  if(!Array.isArray(r))throw'not array';
});

// ====== 4. PreToolRiskAnalyzer ======
assert('C','PreToolRiskAnalyzer basic',function(){
  var P=require('./src/core/PreToolRiskAnalyzer');
  var p=new P();
  var r=p.analyze('ReadFile',{path:'src/test.js'},[]);
...
assert('C','PreToolRiskAnalyzer BLOCK delete critical',function(){
  var P=require('./src/core/PreToolRiskAnalyzer');
  var p=new P();
  var r=p.analyze('DeleteFile',{path:'src/core/BrainSystem.js'},[]);
...
assert('C','PreToolRiskAnalyzer WARN write config',function(){
  var P=require('./src/core/PreToolRiskAnalyzer');
  var p=new P();
  var r=p.analyze('WriteFile',{path:'.opencode/brain.config.json'},[]);
...
assert('C','PreToolRiskAnalyzer ALLOW normal write',function(){
  var P=require('./src/core/PreToolRiskAnalyzer');
  var p=new P();
  var r=p.analyze('WriteFile',{path:'src/test.js'},[]);
...
assert('C','BrainSystem 导出 Phase C 模块',function(){
  var bs=require('./src/core/BrainSystem');
...
assert('C','PRE_TOOL_USE hook 注册',function(){
  ...
  var h=hooks.globalHookRegistry.getHooks(hooks.HookEvents.PRE_TOOL_USE);
  var found=h.some(function(x){return x.name==='brain-risk-analyzer'});
  bs.disconnectHooks();
...
assert('R','BRAIN_DISABLE',function(){
  var r=cp.spawnSync('node',['brain-bridge.js','test'],{encoding:'utf8',timeout:10000,env:Object.assign({},process.env,{BRAIN_DISABLE:'1'})});
  if(JSON.parse(r.trim()).error!=='disabled')throw'not disabled';
});
assert('R','--status',function(){var j=bb('--status');if(!j.enabled)throw'no enabled'});
assert('R','--help',function(){var j=bb('--help');if(!j.usage)throw'no usage'});
assert('R','--diagnose',function(){var j=bb('--diagnose error');if(!j.matches)throw'no matches'});
assert('R','--pending',function(){var j=bb('--pending');if(!j.pending)throw'no pending'});

assert('R','brain-entry.js',function(){
  var r=cp.spawnSync('node',['brain-entry.js','--status'],{encoding:'utf8',timeout:30000});
  if(!r.includes('v22.1'))throw'no v22.1';
});
assert('R','brain-context.js',function(){
  var r=cp.spawnSync('node',['brain-context.js'],{encoding:'utf8',timeout:30000});
  if(!r.includes('BRAIN_SUMMARY'))throw'no summary';
});
assert('R','brain-decision.js',function(){
  var r=cp.spawnSync('node',['brain-decision.js','code'],{encoding:'utf8',timeout:30000});
  if(!r.includes('lesson'))throw'no lesson';
});

assert('R','LessonLibrary quiet',function(){var L=require('./src/core/LessonLibrary');var l=new L({quiet:true});if(l.lessons.length<41)throw'<41'});
assert('R','CircuitBreaker',function(){var C=require('./src/core/CircuitBreaker');var c=new C({maxRetries:1});c.recordFailure();if(c.state!=='CLOSED')throw'not closed';c.reset();if(c.state!=='OPEN')throw'not open'});
assert('R','LoopGuard',function(){var L=require('./src/core/LoopGuard');var l=new L({maxPerMinute:1});l.check('t','x');l.check('t','x');if(!l.check('t','x').tripped)throw'not tripped'});
assert('R','AGENTS.md',function(){var c=fs.readFileSync('AGENTS.md','utf8');if(!c.includes('Phase C'))throw'no Phase C';if(!c.includes('动态决策注入'))throw'no dynamic'});
assert('R','config 完整',function(){var c=JSON.parse(fs.readFileSync('.opencode/brain.config.json','utf8'));['enabled','circuitBreaker','loopGuard','audit','backup','learner','diagnose','daemon'].forEach(function(k){if(c[k]===undefined)throw'missing '+k})});
...
assert('C','AutoDiagnose _tokenize CJK preserved',function(){
  var AD=require('./src/core/AutoDiagnose');
  var a=new AD();
  var tokens=a._tokenize('出现 安全 问题');
  ...
  var tokens2=a._tokenize('this is a test error');
...
assert('C','CircuitBreaker exact time boundary',function(){
  var C=require('./src/core/CircuitBreaker');
  var c=new C({maxRetries:1,resetAfterMs:10});
...
assert('C','LoopGuard exact boundary not tripped',function(){
  var L=require('./src/core/LoopGuard');
  var l=new L({maxPerMinute:3});
  var r1=l.check('src','write');
  if(r1.tripped)throw'1st call tripped';
  var r2=l.check('src','write');
  if(r2.tripped)throw'2nd call tripped';
  var r3=l.check('src','write');
  if(r3.tripped)throw'3rd call (exact limit) tripped';
  var r4=l.check('src','write');
...
assert('C','DecisionTracker auto-prune at 500',function(){
  var DT=require('./src/core/DecisionTracker');
  var d=new DT({maxEntries:10});
  for(var i=0;i<12;i++)d.record({input:'t'+i,taskType:'test'});
  var h=d.getHistory(20);
...
assert('C','PreToolRiskAnalyzer delete config warns',function(){
  var P=require('./src/core/PreToolRiskAnalyzer');
  var p=new P();
  var r=p.analyze('DeleteFile',{path:'.opencode/brain.config.json'},[]);
...
// ====== 清洁 ======
try{var lib=JSON.parse(fs.readFileSync('.opencode/lessons.json','utf8'));lib.lessons=lib.lessons.filter(function(x){return x.lesson!=='test'&&x.lesson!=='Test lesson'});fs.writeFileSync('.opencode/lessons.json',JSON.stringify(lib,null,2))}catch(e){}
try{var p='.opencode/evolution/pending-lessons.json';if(fs.existsSync(p)){var pd=JSON.parse(fs.readFileSync(p,'utf8'));var before=pd.length;pd=pd.filter(function(x){return x.source!=='lesson-learner'&&x.input.indexOf('test')===-1});if(pd.length!==before)fs.writeFileSync(p,JSON.stringify(pd,null,2))}}catch(e){}
