...
> 实证①：视觉模型有顶 → 分层能力（DOM 精确提取 + 网络 API + 模型只做语义补充）。
> 实证②（语义短板）：moondream 对复杂/中文页面脆弱（误判/崩溃）→ 换思路：网页语义 = DOM 事实 + qwen 文本总结（准且稳），moondream 只留纯图。
> "模型有顶，组合方法无顶"——已记入教训库成功经验。

> **知道不足就要弥补（2026-09-18 用户教导，成功经验）**：
> 诚实列出不足只是起点，不是完成——"知道不足 + 当场弥补（行动/换思路/工具化）"才算完成。
> 实证：语义短板诚实承认 → 换思路（DOM+qwen）→ 真正解决。已记入教训库成功经验。

> **学而后用，戒纸上谈兵（2026-09-18 低级错误警戒）**：
> 调研/学习的目的是改进，不是报告——调研完必须**落地能落地的**，光报告 = 纸上谈兵。
> 错误实证：调研 GitHub 手眼项目 → 报告了 stealth/真实浏览器/MCP/视觉方向 → 只落地 stealth，
> 其他标"未来方向"（用"记录方向"掩盖"没落地"）——明知"报告≠完成"仍犯，是低级错误。
> 警戒：① 调研后立即落地能落地的（不标"未来"当掩护）；② "学到了"≠"做到了"，报告充实≠完成；
> ③ "学习→落地"是一体，不是两个阶段。已记入教训库纠错教训。

> **执行驱动的发现（2026-09-18 用户"模拟真实使用看缺什么"教导，成功经验）**：
> 找不足/找缺口时，**不用"想"（盘点/假设），用"真实模拟使用"**——跑真实任务让执行暴露认知盲区。
> 实证：手眼 5 个缺口没一个靠想能想到，全是跑真实任务暴露的（执行受现实检验，想法受认知局限）。
> "不看有什么能做什么，通过模拟真实使用看缺什么"——已记入教训库成功经验。
...
> - `node scripts/browser-eye.js <url> --search="q" --click="sel" --after=extract` —— 交互（搜索/点击/填表）
> - `node scripts/browser-eye.js --image=<path> [--task=describe|ocr|identify]` —— 直接理解本地图片
> - 底层：`src/agent/BrowserAgent.js`（手）+ `src/skills/executors/VisionExecutor.js`（眼）
> **分层使用（诚实 + 换思路）**：网页文字/链接/表单用 `--extract`/`--text`（DOM 精确，零模型）；
> 网页语义理解用 **DOM 事实 + qwen2.5:7b 中文总结**（比 moondream 准且稳——moondream 盲看曾把
> Wikipedia 误判为搜索引擎、对中文崩溃）；moondream 只用于纯图片（`--image`）。
> **用途**：用户请求"看看某网页/某图/检查某站点"时，用此工具执行（确定性优先，模型只做补充）。
