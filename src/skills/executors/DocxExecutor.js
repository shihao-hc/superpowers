...
const { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell, ImageRun,
  ...
  TableOfContents, ExternalHyperlink: _ExternalHyperlink, Bookmark: _Bookmark } = require('docx');
...
class DocxExecutor {
  ...
  static async execute(inputs) {
    ...
    try {
      ...
      throw new Error(`DocxExecutor failed: ${error.message}`, { cause: error });
    }
  }
  ...
  static async createDocument(inputs) {
    const { templatePath: _templatePath, data, content, filePath, title, author, subject } = inputs;
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `docx-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.docx`);
  ...
  static async createDocumentWithHeadings(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `docx-heading-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.docx`);
  ...
  static async createDocumentWithTable(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `docx-table-${Date.now()}-${Math.random().toString(36).substr(9, 9)}.docx`);
  ...
  static async createDocumentWithImage(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `docx-image-${Date.now()}-${Math.random().toString(36).substr(9, 9)}.docx`);
  ...
  static async createReport(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `docx-report-${Date.now()}-${Math.random().toString(36).substr(9, 9)}.docx`);
