...
class PdfExecutor {
  ...
  static async execute(inputs) {
    ...
    try {
      ...
      throw new Error(`PdfExecutor failed: ${error.message}`, { cause: error });
    }
  }
  ...
  static async createPDF(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `pdf-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.pdf`);
  ...
  static async createPDFWithForm(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `pdf-form-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.pdf`);
    ...
    if (formFields && Array.isArray(formFields)) {
      formFields.forEach((field) => {
        const { label, type = 'text', required = false, defaultValue: _defaultValue = '', options: _options } = field;
  ...
  static async createPDFWithTable(inputs) {
    const { title, tableData, headers, filePath, width, height, columnWidths: _columnWidths } = inputs;
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `pdf-table-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.pdf`);
  ...
  static async createPDFReport(inputs) {
    const { title, sections, author, date, filePath, includeToc: _includeToc, abstract, conclusion } = inputs;
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `pdf-report-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.pdf`);
  ...
  static async createPDFInvoice(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `pdf-invoice-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.pdf`);
