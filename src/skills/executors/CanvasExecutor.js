...
const { createCanvas, loadImage, registerFont: _registerFont } = require('canvas');
...
class CanvasExecutor {
  ...
  static async execute(inputs) {
    ...
    try {
      ...
      throw new Error(`CanvasExecutor failed: ${error.message}`, { cause: error });
    }
  }
  ...
  static async createCanvas(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `canvas-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.${format}`);
  ...
  static async createCanvasWithElements(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `canvas-grid-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.png`);
  ...
  static async createChart(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `chart-${chartType}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.png`);
  ...
  static async createIcon(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `icon-${iconType}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.png`);
  ...
  static async createBanner(inputs) {
    ...
    const outputPath = filePath ? path.join(uploadsDir, path.basename(filePath)) : path.join(uploadsDir, `banner-${Date.now()}-${Math.random().toString(36).substr(2, 9)}.png`);
   ...
   */
  static drawPattern(ctx, width, height, pattern, _baseColor) {
  ...
  static async editCanvas(inputs) {
    const { filePath, elements = [], backgroundColor: _backgroundColor } = inputs;
