...
class StorageAdapter {
  ...
  async upload(file, options = {}) {
    const {
      ...
      contentType,
      metadata = {},
      expiresIn: _expiresIn = 365 * 24 * 60 * 60 // 1 year default
    } = options;
  ...
  async _uploadToMinIO(file, key, contentType, _metadata) {
    const buffer = Buffer.isBuffer(file) ? file : file.buffer;
  ...
  async _deleteFromLocal(key) {
    ...
    try {
      ...
      warnLog(`StorageAdapter._deleteFromLocal failed: ${err.message}`, { key, filePath });
    }
...
class MultiFormatExporter {
  ...
  async _exportJSON(data, _options) {
  ...
  async _exportCSV(data, _options) {
    let csv = '';
  ...
  async _exportMarkdown(data, _options) {
    let md = '';
  ...
  async _exportHTML(data, _options) {
    let html;
...
<body>
  ...
  async _exportPDF(data, _options) {
