...
const Validation = {
  ...
  sanitizeString(str, maxLength = 1000) {
    ...
    return str.slice(0, maxLength).replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, ''); // eslint-disable-line no-control-regex
  },
...
class EnhancedSkillsApi {
  ...
  _bindRoutes() {
    ...
    this.router.post('/preview/create',
      ...
      async (req, res) => {
        try {
          const { content, filename, type: _type } = req.body;
          const file = req.file;
    ...
    this.router.post('/export/file', this.upload.single('file'), async (req, res) => {
      try {
        const file = req.file;
        const { format: _format, metadata } = req.body;

        if (!file) {
          return res.status(400).json({ error: 'File is required' });
        }

        const safeFilename = Validation.sanitizeString(path.basename(file.originalname || 'untitled'), 255);
        const result = await this.exporter.storage.upload(file.buffer, {
          key: `exports/${safeFilename}`,
