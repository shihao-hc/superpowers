...
class AutonomousLearning {
  ...
  learn(interaction = {}) {
    ...
    // 4. 主动学习
    const learning = this._activeLearn(patterns);

    // 5. 反馈改进
    this._improve(learning);

    return { gaps, patterns, learning, timestamp: Date.now() };
  }
  ...
  _discoverGaps(interaction) {
    ...
    if (interaction.confidence < 0.7) {
      gaps.push({ type: 'low_confidence', area: interaction.intent, urgency: 'high' });
    }

    if (interaction.error) {
      gaps.push({ type: 'error', area: 'general', urgency: 'high' });
    }

    // 主动检查
    if (this._learningHistory.length % 10 === 0) {
      gaps.push({ type: 'periodic', area: 'review', urgency: 'medium' });
    }
  ...
  _discoverPatterns() {
    // 分析历史交互
    if (this._learningHistory.length < 3) {return [];}

    const recent = this._learningHistory.slice(-3);
   ...
   * 主动学习
   */
  _activeLearn(patterns) {
    const learnings = [];

    // 从模式学习
    for (const pattern of patterns) {
      if (pattern.type === 'repeated_intent') {
        learnings.push({
  ...
  getStatus() {
    return {
      ...
      gapsFound: this._knowledgeGaps.length,
      patternsDiscovered: this._discoveredPatterns.length
    };
  }
}
...
module.exports.AutonomousLearning = AutonomousLearning;
