...
class FileCache {
  ...
  async ensureDir(dir) {
    try {
      ...
      this.logger?.debug(`ensureDir skipped: ${e.message}`);
    }
  }

  async get(key) {
    const { filePath, dir: _dir } = this._getFilePath(key);
  ...
  async clear() {
    ...
    const clearDir = async (dir) => {
      try {
        ...
        this.logger?.debug(`clearCache skip: ${e.message}`);
      }
    };
  ...
  async getSize() {
    ...
    const calcSize = async (dir) => {
      try {
        ...
        this.logger?.debug(`getSize skip: ${e.message}`);
      }
    };
...
class RedisCache {
  ...
  async set(key, value, ttl = this.defaultTTL) {
    ...
    try {
      ...
      this.logger?.debug(`Redis set failed: ${e.message}`);
    }
  }
  ...
  async get(key, _options = {}) {
    ...
    if (this.enabled.l3) {
      try {
        ...
        this.logger?.debug(`MultiLevelCache get L3 failed: ${e.message}`);
      }
    }
