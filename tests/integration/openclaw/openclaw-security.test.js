...
describe('OpenClaw Security', () => {
  ...
  describe('原型污染防护', () => {
    const { isPrototypePollutionSafe } = require('../../../src/integrations/openclaw/OpenClawClient');
  ...
  describe('速率限制', () => {
    const { UnifiedRateLimiter } = require('../../../src/rateLimiter/UnifiedRateLimiter');

    test('应该在限制内允许请求', () => {
      const limiter = new UnifiedRateLimiter({ defaultLimit: 100, defaultWindow: 60000 });
      const ip = '192.168.1.1';

      for (let i = 0; i < 50; i++) {
        expect(limiter.checkFixedWindow(ip, 100, 60000).allowed).toBe(true);
      }
    });
    ...
    test('应该拒绝超过限制的请求', () => {
      ...
      for (let i = 0; i < 100; i++) {
        limiter.checkFixedWindow(ip, 100, 60000);
      }

      expect(limiter.checkFixedWindow(ip, 100, 60000).allowed).toBe(false);
    });
    ...
    test('不同 IP 应该有独立限制', () => {
      ...
      for (let i = 0; i < 100; i++) {
        limiter.checkFixedWindow('192.168.1.3', 100, 60000);
      }

      expect(limiter.checkFixedWindow('192.168.1.4', 100, 60000).allowed).toBe(true);
    });

    test('应该支持令牌桶算法', () => {
      const limiter = new UnifiedRateLimiter({ defaultLimit: 10, defaultWindow: 60000 });

      for (let i = 0; i < 5; i++) {
        const result = limiter.checkTokenBucket('192.168.1.5', 5, 1);
        expect(result.allowed).toBe(true);
      }

      const exhausted = limiter.checkTokenBucket('192.168.1.5', 5, 1);
      expect(exhausted.allowed).toBe(false);
      expect(exhausted.remaining).toBe(0);
    });

    test('应该支持滑动窗口算法', () => {
      const limiter = new UnifiedRateLimiter({ defaultLimit: 5, defaultWindow: 60000 });

      for (let i = 0; i < 5; i++) {
        expect(limiter.checkSlidingWindow('192.168.1.6', 5, 60000).allowed).toBe(true);
      }

      expect(limiter.checkSlidingWindow('192.168.1.6', 5, 60000).allowed).toBe(false);
    });
  });
