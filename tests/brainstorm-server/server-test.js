...
function waitForReloadMessage(ws, timeout = 3000) {
  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('Timeout waiting for reload')), timeout);
    const handler = (data) => {
      try {
        if (JSON.parse(data.toString()).type === 'reload') {
          clearTimeout(timer);
...
async function waitForStdoutContains(getAccum, pattern, timeout = 3000) {
  const start = Date.now();
  while (Date.now() - start < timeout) {
    if (getAccum().includes(pattern)) {return;}
    await sleep(50);
  }
  throw new Error(`Stdout did not contain "${pattern}" within ${timeout}ms`);
}

async function waitForHttpResponse(url, checkFn, timeout = 3000) {
  const start = Date.now();
  let lastError;
  while (Date.now() - start < timeout) {
    try {
      const res = await fetch(url);
      if (checkFn(res)) {return res;}
    } catch (e) { lastError = e; }
    await sleep(100);
  }
  throw lastError || new Error(`HTTP condition not met within ${timeout}ms`);
}

async function waitForFileGone(filePath, timeout = 3000) {
  const start = Date.now();
  while (Date.now() - start < timeout) {
    if (!fs.existsSync(filePath)) {return;}
    await sleep(20);
  }
  throw new Error(`File still exists after ${timeout}ms: ${filePath}`);
}
...
async function runTests() {
  ...
  try {
    ...
    await test('serves full HTML documents as-is (not wrapped)', async () => {
      ...
      const res = await waitForHttpResponse(
        `http://localhost:${TEST_PORT}/`,
        (r) => r.body.includes('<h1>Custom Page</h1>')
      );
    ...
    await test('wraps content fragments in frame template', async () => {
      ...
      const res = await waitForHttpResponse(
        `http://localhost:${TEST_PORT}/`,
        (r) => r.body.includes('indicator-bar')
      );
    ...
    console.log('\n--- WebSocket Communication ---');
    // Settle file watcher debounce timers before WebSocket tests
    await sleep(500);
    ...
    await test('relays user events to stdout with source field', async () => {
      ...
      await waitForStdoutContains(() => stdoutAccum, '"source":"user-event"');
    ...
    await test('handles multiple concurrent WebSocket clients', async () => {
      ...
      await Promise.all([
        waitForReloadMessage(ws1),
        waitForReloadMessage(ws2)
      ]);
    ...
    await test('logs screen-updated on file change', async () => {
      ...
      await waitForStdoutContains(() => stdoutAccum, 'screen-updated');
    });
