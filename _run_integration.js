// Start server and run integration tests inline
process.env.NODE_ENV = 'test';
process.env.API_KEY = 'test-key';
process.env.PORT = '3095';
process.env.TEST_BASE_URL = 'http://localhost:3095';
...
async function runTests() {
  // Check server health
  const health = await httpGet('http://localhost:3095/api/health');
  console.log('Health:', health.status, health.body);

  // Now run vitest via fork (not execSync + npx) to avoid Windows cmd.exe hanging
  ...
  const testEnv = Object.assign({}, process.env, {
    TEST_BASE_URL: 'http://localhost:3095',
    API_KEY: 'test-key'
  });
...
const server = app.listen(3095, '0.0.0.0', runTests);
